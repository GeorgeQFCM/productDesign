<template>
  <section>
    ...
    <el-row>
      <el-table border highlight-current-row height="700" size="small" :data="tableData" stripe>
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-table :data="props.row.list" stripe border highlight-current-row size="mini"
              :header-cell-style="{background:'#d8effb',color:'#555'}">
              <el-table-column prop="type" label="类型" show-overflow-tooltip align="center">
                <template slot-scope="scope">
                  <span
                    :class="scope.row.number!==0 && scope.row.suggest=='紧急备货'?'textColor':''">{{scope.row.type}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="number" label="数量" show-overflow-tooltip align="center">
                <template slot-scope="scope">
                  <span
                    :class="scope.row.number!==0 && scope.row.suggest=='紧急备货'?'textColor':''">{{scope.row.number}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="day" label="可售天数" show-overflow-tooltip align="center">
                <template slot-scope="scope">
                  <span
                    :class="scope.row.number!==0 && scope.row.suggest=='紧急备货'?'textColor':''">{{scope.row.number /props.row.averageDailySales}}</span>
                </template>
              </el-table-column>
              <el-table-column label="建议操作" show-overflow-tooltip align="l">
                <template slot-scope="scope">
                  <span v-if="props.row.stockNum /props.row.averageDailySales<KSday"
                    style="font-size: 15px;font-family: inherit;">建 议 </span>
                  <button style="border-bottom: 1px solid; padding: 1px;" class='bttn-stretch bttn-md bttn-primary'
                    @click="suggestClick(scope.row.suggest,props.$index,scope.row.number)">{{scope.row.suggest}}
                  </button>
                </template>
              </el-table-column>
            </el-table>
