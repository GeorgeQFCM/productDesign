<template>
  <section>
    ...
    <el-row>
      <el-col :span="23">
        <el-form size="small" inline>
          ...
          <el-form-item>
            <el-button icon="el-icon-search" type="primary" @click="autoA">自动分析</el-button>
          </el-form-item>
        </el-form>
      </el-col>
    ...
    <el-table :data="dataList" height="69vh" size="small" @cell-click="dblhandleCurrentChange" ref="dataListRef"
      ...
      <el-table-column prop="department" label="事业部" width="100" align="center">
        <!-- <template slot-scope="scope">
          <span v-if="scope.row.index === tabClickIndex && tabClickLabel === '事业部'">
            <el-select v-focus size="small" filterable v-model="scope.row.department"
              @change="selectChange(scope.row,scope.column.label)">
              <el-option v-for="item in departmentoptions" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </span>
          <span v-else :style="{color:scope.row.departmentisEdit==true?'red':'#606266'}">{{scope.row.department}}</span>
        </template> -->
      </el-table-column>
      <el-table-column prop="team" label="Team" width="100" align="center">
        <!-- <template slot-scope="scope">
          <span v-if="scope.row.index === tabClickIndex && tabClickLabel === 'Team'">
            <el-select v-focus size="small" filterable v-model="scope.row.team"
              @change="selectChange(scope.row,scope.column.label)">
              <el-option v-for="item in teamoptions" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </span>
          <span v-else :style="{color:scope.row.teamisEdit==true?'red':'#606266'}">{{scope.row.team}}</span>
        </template> -->
      </el-table-column>
      <el-table-column prop="platform" label="平台" width="100" align="center">
        <!-- <template slot-scope="scope">
          <span v-if="scope.row.index === tabClickIndex && tabClickLabel === '平台'">
            <el-select v-focus size="small" filterable v-model="scope.row.platform"
              @change="selectChange(scope.row,scope.column.label)">
              <el-option v-for="item in platformoptions" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </span>
          <span v-else :style="{color:scope.row.platformisEdit==true?'red':'#606266'}">{{scope.row.platform}}</span>
        </template> -->
      </el-table-column>
      <el-table-column prop="area" label="区域" width="100" align="center">
        <!-- <template slot-scope="scope">
          <span v-if="scope.row.index === tabClickIndex && tabClickLabel === '区域'">
            <el-select v-focus size="small" filterable v-model="scope.row.area"
              @change="selectChange(scope.row,scope.column.label)">
              <el-option v-for="item in areaoptions" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </span>
          <span v-else :style="{color:scope.row.areaisEdit==true?'red':'#606266'}">{{scope.row.area}}</span>
        </template> -->
      </el-table-column>
      <el-table-column prop="transport" label="运输方式" width="100" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.index === tabClickIndex && tabClickLabel === '运输方式'">
            <el-select v-focus size="small" filterable v-model="scope.row.transport"
              @change="selectChange(scope.row,scope.column.label)">
              <el-option v-for="item in options" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        departmentoptions: ['事业一部', '事业二部'],
        teamoptions: ['Team1', 'Team2'],
        platformoptions: ['Amazon', 'Rakuten'],
        areaoptions: ['UK', 'US'],
        options: ['空运', '海运'],
        queryFrom: {
          page: 1,
    ...
    methods: {
      ...
      selectChange(row, label) {
        console.log(row);
        // console.log(row.transport);
        // row.transportisEdit = true
        switch (label) {
          case "事业部":
            row.departmentisEdit = true
            break;
          case "Team":
            row.teamisEdit = true
            break;
          case "平台":
            row.platformisEdit = true
            break;
          case "区域":
            row.areaisEdit = true
            break;
